...
folder_name       = 'epoch' + '_' + learning_mode + '_' + mode_name  + '_' + step_size_version + '_' + projection + '_' + coupling + '_' + str(nr_channel) + 'channel'
...
def get_constraint(nr_sample=1000):
    for i in range(nr_sample):
        ...
        theta = theta - theta[:, 0].reshape(-1, 1)  # relative trajectory
        # RG.GetPlot(p, theta, t_list=np.array([0.0, t, t+1.5, t+1.7]))
        Xi = get_Xi(theta)
        # tt = get_peak(theta)
        tt = list(range(0, theta.shape[1], 5))
        if i == 0:
            Cons = Xi[tt, :]
        else:
...
pre_round = 1
Xi_total = [None] * 3
Cons = get_constraint(nr_sample=300)
...
for i_epoch in range(nr_epoch):
    ...
    for i_it in range(len(train_index)):
        ...
        nr_round = i_epoch * len(train_index) + i_it + pre_round # + 1
        for dof in range(len(b_list)):  # update the linear model b
            '''
            ...
            # if i_epoch == 0 and dof == 0:
                # Xi_total[dof]                                             = np.concatenate((Xi_total[dof], Xi), axis=0)
                # Cons = np.concatenate((Cons, Xi), axis=0)
            sk                                                        = get_step_size(nr_round, dof, step_size_version=step_size_version)
            b_list[dof]                                               = get_update(b_list[dof], sk, hessian, gradient, Xi)
            b_list[dof]                                               = get_projection(dof, b_list[dof], Cons, projection=projection)
