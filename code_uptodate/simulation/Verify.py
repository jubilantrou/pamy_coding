...
# dof: 0 1 2 3
delta_d_list = np.array([1e-9, 1e-10, 1e-8, 1e-9]) 
delta_y_list = np.array([1e-3, 1e-2, 1e-3, 1e-3])
delta_w_list = np.array([1e-9, 1e-10, 1e-8, 1e-9])
delta_ini_list = np.array([1e-7, 1e-7, 1e-7, 1e-7])
# %%
# choose one trajectory to train the model
# suffix = ".txt"
# name_desired_tajectory = "SmoothSignal_1"
# name_file = "F_" + name_desired_tajectory + "_a_" + "_i_" + str(number_iteration)
# # the path of files
# path_of_trajectory = "/home/hao/Desktop/Hao/TestSignal/" + name_desired_tajectory + suffix
# path_of_file = "/home/hao/Desktop/Hao/data/" + name_file + suffix
# # %% generate the input for each dof
# # import the desired trajectory from the file
# fid = open(path_of_trajectory)
# # time length of the disired signal
# T = float( fid.readline().strip() )
# # number of points of the disired signal
# Ns = int( fid.readline().strip() )

# y_desired = np.array([])
# for _ in range(0,Ns):
#     y_desired = np.append( y_desired, amplitude_input_list * float(fid.readline().strip()) )
# y_desired = y_desired.reshape(-1, len(dof_list)).T
( T, y_desired ) = RealTrajectoryGeneration.RacketTrajectory()
Ns = int( T * fs + 1 )
# time stamp
...
pid_list = [[-3.505924158687806e+04, -3.484022215671791e+05, -5.665386729745434e+02],
...
pressure_min = [0, 0, -4818, 0]
pressure_max = [0, 0, 4818, 0]
...
# %%
path_of_file = "/home/hao/Desktop/Hao/data/" + 'play' + '.txt'
...
dof = 0
# %%
y_desired = y_history[0][dof]
y_list = []
ff_list = []
disturbance_list = []
for index in range( len( ff_history ) - 1 ):
    y_list.append( y_history[index+1][dof] )
    ff_list.append( ff_history[index][dof] )
    disturbance_list.append( disturbance_history[index][dof] )
# %%
y_est = []
d_bar = []
for index in range( len( y_list )):
    y_est_temp = np.dot( RealRobot.Optimizer_list[dof].Bu, ff_list[index].reshape(-1, 1) ) +\
                 np.dot( RealRobot.Optimizer_list[dof].Bd, disturbance_list[index].reshape(-1, 1) )
    d_bar_temp = np.dot( RealRobot.Optimizer_list[dof].Bd, disturbance_list[index].reshape(-1, 1) )
    d_bar_temp = np.delete(d_bar_temp, 0, axis=0)
    y_est_temp = np.delete(y_est_temp, 0, axis=0)
    y_est.append( y_est_temp )
    d_bar.append( d_bar_temp )

y_diff = y_desired - y_list[0]

# %%
row_list = [1, 20, 39]
# row_list = np.arange(0, 39)
line = []
Location = 'upper left'
...
line_temp, = plt.plot(t_stamp_u, y_desired * 180 / math.pi, label=r'Desired Trajectory', linestyle='--', linewidth=2)
line.append( line_temp )
# line_temp, = plt.plot(t_stamp_u, ( y_diff - y_diff[0] ) * 180 / math.pi, label=r'disturbance', linewidth=1)
# line.append(line_temp)
for index in row_list:
    line_temp, = plt.plot(t_stamp_u, ( y_list[index] - y_list[index][0] )* 180 / math.pi, label=r'Iteration {}, output'.format(index), linewidth=1)
    line.append(line_temp)
    line_temp, = plt.plot(t_stamp_u, ( y_est[index] )* 180 / math.pi, label=r'Iteration {}, estimated'.format(index), linewidth=0.3)
    line.append(line_temp)
    # line_temp, = plt.plot(t_stamp_u, ( d_bar[index] ) * 180 / math.pi, label=r'Iteration {}, d transformed'.format(index), linewidth=0.3)
    # line.append(line_temp)
...
plt.figure(2)
plt.xlabel(r'Time $t$ in s')
plt.ylabel(r'Feedforward $p$ in kPa')
for index in row_list:
    line_temp, = plt.plot(t_stamp_u, ff_list[index], label=r'Iteration {}'.format(index), linewidth=1)
    line.append(line_temp)
