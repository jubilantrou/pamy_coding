'''
This script is used to store the configurations of 
the simulator and the real Pamy.
'''
# %%
import numpy as np
...
# ----------------------------------------------------------------------------------------------------------------
'''
!!!!! Last Used Parameters Before Modification !!!!!
...
obj:                 sim
'''
sim_inverse_model_num = [[-686139.776350245,	1818668.61077509,	-1607353.20010400,	473739.600198047],
...
sim_inverse_model_num = FulfillZeros(sim_inverse_model_num)

sim_inverse_model_den = [[1,	-0.793108906364425,	0.572350454862304,	-0.153007415376520],
...
sim_inverse_model_den = FulfillZeros(sim_inverse_model_den)

sim_ndelay_list = np.array([2, 3, 3, 3])
sim_order_num_list = np.array([3, 3, 2, 3])
sim_order_den_list = np.array([3, 3, 2, 3])

sim_model_num = [[-9.21270826464792e-07,	3.91751500835520e-07,	-3.83172453373049e-07],
...
sim_model_num = FulfillZeros(sim_model_num)

sim_model_den = [[1,	-2.65058035324677,	2.34260314808439,	-0.690441826180068],
...
sim_model_den = FulfillZeros(sim_model_den)   

sim_model_ndelay_list = [2, 3, 3, 3]
sim_model_num_order   = [2, 3, 2, 2]
sim_model_den_order   = [3, 3, 2, 3]
# ----------------------------------------------------------------------------------------------------------------
...
# for the simulator
sim_anchor_ago_list = np.array([19300, 17500, 17100, 16000])
sim_anchor_ant_list = np.array([19900, 22000, 16900, 17400])

sim_ago_min_list = np.array([15000, 15000, 13000, 13000])
sim_ago_max_list = np.array([24900, 21400, 20900, 18900])
sim_ant_min_list = np.array([16000, 16000, 13000, 13000])
sim_ant_max_list = np.array([24900, 24900, 20900, 21800])

sim_ago_pressure_max = sim_ago_max_list - sim_anchor_ago_list
sim_ago_pressure_min = sim_ago_min_list - sim_anchor_ago_list
sim_ant_pressure_max = sim_ant_max_list - sim_anchor_ant_list
sim_ant_pressure_min = sim_ant_min_list - sim_anchor_ant_list
sim_pressure_max = [min([sim_ago_pressure_max[i],-sim_ant_pressure_min[i]]) for i in range(len(dof_list))]
sim_pressure_min = [max([sim_ago_pressure_min[i],-sim_ant_pressure_max[i]]) for i in range(len(dof_list))]
sim_pressure_limit = [min([sim_ago_pressure_max[i],-sim_ago_pressure_min[i],sim_ant_pressure_max[i],-sim_ant_pressure_min[i]]) for i in range(len(dof_list))]
...
sim_pid_list = np.array([[-8368.8*0.8,  -66950.4*0.8*0.8, -261.525],
...
'''
def build_pamy(frontend=None, obj=None):
    if obj=='real':
        ...
        Pamy = Robot(frontend, dof_list, sim_model_num, sim_model_den,
                    sim_model_num_order, sim_model_den_order, sim_model_ndelay_list,
                    sim_inverse_model_num, sim_inverse_model_den, sim_order_num_list, sim_order_den_list,
                    sim_ndelay_list, sim_anchor_ago_list, sim_anchor_ant_list, strategy_list, sim_pid_list,
                    delta_d_list, delta_y_list, delta_w_list, delta_ini_list,
                    sim_pressure_min, sim_pressure_max, weight_list)
    return Pamy
# %%
def build_geometry():
