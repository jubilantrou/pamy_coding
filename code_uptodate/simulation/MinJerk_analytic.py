...
def steps_from_time(T, dt):
    ...
    """
    return int(T / dt)  # np.arange(0,T,dt)

def PathPlanning(dt, tt, xx, uu, aa, smooth_acc=True):
  ...
  """
  # xx = [get_minjerk_trajectory(dt, tt, xx[i], uu[i], smooth_acc=smooth_acc) for i in range(len(xx))]
  xxx = [None] * xx.shape[0]
  vvv = [None] * xx.shape[0]
  aaa = [None] * xx.shape[0]
  jjj = [None] * xx.shape[0]
  ...
  for i in range(len(xx)):
    xxx[i], vvv[i], aaa[i], jjj[i] = get_minjerk_trajectory(dt, tt, xx[i], uu[i], aa[i], smooth_acc=smooth_acc)
  # xxx = [get_minjerk_trajectory(dt, tt, xx[i], uu[i], aa[i], smooth_acc=smooth_acc) for i in range(len(xx))]
  xxx = np.array(xxx)
  vvv = np.array(vvv)
  aaa = np.array(aaa)
  jjj = np.array(jjj)
...
def get_minjerk_trajectory(dt, tt, xx, uu, aa, smooth_acc=False):
  ...
  # Initialization
  T_whole = tt[-1] - tt[0]
  N_Whole = steps_from_time(T_whole, dt) + 1
  ...
  for i in range(N-1):
    ...
    # x = get_min_jerk_trajectory(dt, t0, t1, x0, x1, u0, u1, a_ta=a_ta, a_tb=a_tb)
...
if __name__ == "__main__":
  ...
  xxx, vvv, aaa, jjj = PathPlanning(dt, tt, xx, uu, aa, smooth_acc)
