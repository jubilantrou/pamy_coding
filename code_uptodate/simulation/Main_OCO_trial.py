...
paras = get_paras()
if paras.obj != PAMY_CONFIG.obj:
    raise ValueError("Make sure the value of obj in paras and the one in PAMY_CONFIG are the same!")
...
#                                                                 height=paras.fb_input_size, width=1, filter_size=None, device=device, hidden_size=[16], linear=True)
...
while True:
    ...
    disturb_datapoint = get_datapoints_pro(aug_data=disturb_aug_ref_traj, ref=update_point_index_list, window_size=(paras.h_l+paras.h_r+1), ds=paras.ds, device=device, nn_type=paras.nn_type)
    # disturb_datapoint = get_datapoints(aug_data=get_compensated_data(data=Pamy.y_desired, h_l=paras.h_l, h_r=paras.h_r), window_size=(paras.h_l+paras.h_r+1), ds=paras.ds, device=device, nn_type=paras.nn_type)
    ...
    u_add = np.zeros((3,Pamy.y_desired.shape[1]))
    for idx_u_add in range(3):
        u_add[idx_u_add, :] = (np.linalg.pinv(par_G_par_u[idx_u_add])@(Pamy.y_desired[idx_u_add, :].reshape(-1,1)-Bd_list[idx_u_add]@(d[idx_u_add,:].reshape(-1,1)))).reshape(-1)
        # u_add[idx_u_add, :] = (np.linalg.pinv(par_G_par_u[idx_u_add])@(Bd_list[idx_u_add]@(d[idx_u_add,:].reshape(-1,1)))).reshape(-1)
        # u_add[idx_u_add, :] = (np.linalg.pinv(par_G_par_u[idx_u_add])@par_G_par_u[idx_u_add]@(u[idx_u_add,:].reshape(-1,1))).reshape(-1)
    ...
    print('dof 0: {} ~ {}'.format(min(u_add[0,:]),max(u_add[0,:])))
    print('dof 1: {} ~ {}'.format(min(u_add[1,:]),max(u_add[1,:])))
    print('dof 2: {} ~ {}'.format(min(u_add[2,:]),max(u_add[2,:])))
    ...
    # T_vel = np.zeros((Pamy.y_desired.shape[1],Pamy.y_desired.shape[1]))
    # for row in range(1,Pamy.y_desired.shape[1]):
    #     T_vel[row, row-1] = -20
    #     T_vel[row, row] = 20
    # par_l_par_y = [((np.eye(Pamy.y_desired.shape[1])+T_vel.T@T_vel)@((y_out-theta)[i]).reshape(-1,1)).reshape(1,-1) for i in range(3)]
    # idx_amp = [1,1.5,1.5]
    ...
    for dof,flag in enumerate(paras.flag_dof):
        if not flag:
            ...
            temp = -par_G_par_u[dof]@np.linalg.pinv(par_G_par_u[dof])@Bd_list[dof]@par_paid_par_wd[dof]
        ...
        for dof in range(len(par_l_par_y)):
            ...
            if method == 'GD':
                ...
                if disturb:
                    ...
                    delta = (lr[dof]*np.linalg.inv(disturb_hessian_temp[dof]/(i_iter+1))@par_y_par_w[dof].T@par_l_par_y[dof].T).reshape(-1 ,1)
                else:
                    ...
                    # hessian_temp[dof] = par_y_par_w[dof].T@par_y_par_w[dof]+paras.alpha_list[dof]*par_pai_par_w[dof].T@par_pai_par_w[dof]+0.0*np.eye(par_pai_par_w[dof].shape[1])
                    # if i_iter<10:
                    #     delta = (lr[dof]*np.linalg.pinv(sum(hessian_temp[dof])/(i_iter+1))@par_y_par_w[dof].T@par_l_par_y[dof].T).reshape(-1 ,1)
                    # else:
                    #     delta = (lr[dof]*np.linalg.pinv(sum(hessian_temp[dof])/10)@par_y_par_w[dof].T@par_l_par_y[dof].T).reshape(-1 ,1)
                    delta = (lr[dof]*np.linalg.inv(hessian_temp[dof]/(i_iter+1))@par_y_par_w[dof].T@par_l_par_y[dof].T).reshape(-1 ,1)
    ...
    # if (i_iter+1)%35==0:
    #     root_file = '/home/mtian/Desktop/MPI-intern/training_log_temp_'+ str(paras.save_path_num) +'/linear_model/fix_traj/' + str((i_iter+1)//30)
    #     mkdir(root_file)
    #     file = open(root_file + '/log.txt', 'wb')
    #     disturb_datapoint_4_copy = []
    #     for ele in disturb_datapoint:
    #         disturb_datapoint_4_copy.append(ele.cpu().detach().numpy())
    #     pickle.dump(len(disturb_datapoint), file, -1)
    #     pickle.dump(disturb_datapoint_4_copy, file, -1)
