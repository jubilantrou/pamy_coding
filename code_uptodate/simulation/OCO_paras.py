...
def get_paras():
    ...
    parser.add_argument('--lr_list', nargs=3, default=[0.025, 0.1, 0.1], type=float, help='learning rates')
    ...
    parser.add_argument('--save_path_num', default=5081, type=int, help='indicate the sequence number of the file where we restore the results')
    parser.add_argument('--message', default='run pretrained from s.1 starting from iter 201 for comparison', type=str, help='briefly record the purpose of this experiment')
    ...
    parser.add_argument('--ff_model', default='/home/mtian/Desktop/MPI-intern/training_log_temp_507/linear_model_disturb/350/', type=str, help='whether to use pretrained policy blocks as a start')
    ...
    parser.add_argument('--traj_option', default='multiple+updates', type=str, choices=['fixed','multiple','multiple+updates'], help='the description of trajectories used for training')
    parser.add_argument('--skipped_traj_num', default=0, type=int, help='the number of trajectories that will be skipped, which is useful when the comparison will be made strating from a certain iteration')
