...
def CalPath(x_0, x_f, t_0, t_f, step, m, n):
  ...
  for i_point in range( nr_point ):
    for i_dof in range( nr_dof ):
      p[i_dof, i_point] = np.dot(parameter[i_dof, :], CalPositionTerm(i_point*step, m[i_dof], n[i_dof]))
      v[i_dof, i_point] = np.dot(parameter[i_dof, :], CalVelocityTerm(i_point*step, m[i_dof], n[i_dof]))
      a[i_dof, i_point] = np.dot(parameter[i_dof, :], CalAccelerationTerm(i_point*step, m[i_dof], n[i_dof]))
      j[i_dof, i_point] = np.dot(parameter[i_dof, :], CalJerkTerm(i_point*step, m[i_dof], n[i_dof]))
...
def GetPath(x_temp, v_temp, a_temp, t_temp, m, n, step):
  ...
  [p, v, a, j] = CalPath(x_0_list, x_f_list, t_temp[0], t_temp[1], step, m, n)
